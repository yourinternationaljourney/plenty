...
function viewAtHome(){
  ...
  ${all.length?`<section class="card pad pantry-start"><div><div class="eyebrow">Cook from what you have</div><h2 style="margin-top:4px">Turn your pantry into a meal</h2><p class="muted" style="margin-top:6px">Plenty matches recipes to this inventory first, so you can cook without automatically adding a whole new shop.</p><div class="actions"><button class="btn primary" data-action="at-home-recipes" data-max="0">Nothing to buy</button><button class="btn" data-action="at-home-recipes" data-max="1">Up to 1 item</button><button class="btn ghost" data-action="at-home-recipes" data-max="3">Up to 3 items</button></div></div><div class="pantry-summary"><b>${atHomeRecipeMatches(allDiscoverRecipes(),3).length}</b><span>possible recipes using your At Home items</span></div></section>`:''}
...
const ASSISTANT_NOTICE='Plenty provides general planning and informational support, not medical, nutritional or financial advice.';
...
function viewSettings(){const s=S.settings;const gb=s.groupBudgets||{};const sum=GROUPS.reduce((a,k)=>a+(+gb[k]||0),0)+(+s.buffer||0);
  ...
  <div class="grid2">
    ...
    <div class="card pad"><h3 style="margin-bottom:12px">Daily targets & food preferences</h3>
      ...
      <div class="field" style="margin-top:12px"><label for="sDiet">Food preferences for Ask Plenty</label><textarea id="sDiet" data-setting="diet" placeholder="e.g. mostly vegetarian, no mushrooms, high protein, quick weeknight dinners, love Asian flavours">${esc(s.diet||'')}</textarea></div></div>
    ${petsCardHtml()}
    ${dataControlsHtml()}
    ${pwaCardHtml()}
    ${appearanceCardHtml()}
