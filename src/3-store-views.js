...
function viewGroceries(){
  ...
  const line=l=>{const pm=l.priceMemory,trend=pm&&pm.changePct!=null?(pm.changePct>2?`<span class="price-trend up">↑ ${Math.abs(pm.changePct).toFixed(0)}%</span>`:pm.changePct< -2?`<span class="price-trend down">↓ ${Math.abs(pm.changePct).toFixed(0)}%</span>`:''):'';return `<div class="gi ${l.checked?'done':''}"><label><input type="checkbox" data-action="check" data-key="${esc(l.key)}" ${l.checked?'checked':''}><span class="q num">${esc(fmtBase(l.needBase,l.fam))}</span><span class="n">${l.priority?`<span class="pri ${l.priority}" title="${l.priority}"></span> `:''}${esc(l.name)}${l.pack?` <span class="faint small">· ${esc(l.pack)}</span>`:''}<small>${esc(l.sources.join(', '))}${l.atHome?` · ${esc(l.atHome.label)}`:''}${pm?` · Personal estimate from ${pm.samples} receipt${pm.samples===1?'':'s'} (${pm.confidence} confidence)`:l.items.length&&l.items[0].store?` · ${esc(l.items[0].store)}`:''} ${trend}</small></span></label><span class="price num">${l.cost?money(l.cost):''}</span><button class="mini" data-action="line-compare" data-key="${esc(l.key)}" title="View the prices learned from your receipts">History</button>${l.items.length?`<button class="mini" data-action="postpone" data-id="${esc(l.items[0].id)}" title="Skip this week">Postpone</button>`:l.extras?`<button class="mini" data-action="remove-extra" data-id="${esc(l.extras[0].id)}">Remove</button>`:`<button class="mini" data-action="have-line" data-name="${esc(l.name)}" data-cat="${esc(l.category)}" title="I already have this">Have it</button>`}</div>`};
