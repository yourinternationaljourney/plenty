...
## 6. v2.1 — Netlify foundation (accounts, database, blobs, functions)

Added on top of v2.0 without changing the local-first core. GitHub Pages stays deployed as the local-only fallback; Netlify builds the same `dist/` with `PLENTY_CLOUD=netlify`, which adds the Identity widget and the Account card.

- **Auth**: Netlify Identity; Functions use the v1 handler signature so `context.clientContext.user` carries the claims Netlify verified. `requireUser()` is the only source of the user id.
- **Data access**: `netlify/functions/lib/repo.js` defines the repository contract with `MemoryRepo` (tests) and `SqlRepo` (Netlify Database via `@neondatabase/serverless`). Application documents are routed into typed tables (`household_settings`, `pets`, `weekly_plans`, `meals`, `recipes`, `grocery_items`, `pantry_items`, `receipts`, `receipt_line_items`, `price_observations`, `health_profiles`, `health_checkins`, `app_documents`) and rebuilt losslessly from the stored `data` column.
- **Receipt images**: `netlify/functions/lib/blobs.js`; validated by magic bytes and size (5 MB), keyed `receipts/<userId>/<uuid>-<random>`, owner-only, removed with receipt or account.
- **Import/restore**: `POST /api/app-data` with `?preview=1` then confirm; `import_status` prevents accidental duplicate imports; `GET /api/app-data` feeds the existing backup/import machinery to restore into a new local profile. Sync stays manual (`docs/SYNC.md`).
- **Verified by tests** (`tests/netlify.test.js`, in-process): unauthenticated rejection on every endpoint, cross-user access returning 404, receipt and blob ownership, invalid upload rejection (type, size, mismatch), import preview/confirm/duplicate/older-data rules, separate health and price deletion, account deletion cleanup of rows, blobs and the Identity user, migration file structure, retailer interface, netlify.toml and cloud/local build variants.
- **Not verified here**: the `SqlRepo` adapter and `NetlifyBlobs` against a live Netlify site (no site or database exists yet). Treat the deployment as a foundation to verify with the steps in `docs/NETLIFY-SETUP.md`, not as production-ready.

## 7. v2.2 — Supermarket price comparison inside Groceries (2026-09-21)

- **UI**: Groceries gained three internal segments (My list, Compare stores, Search products), a per-line *Compare* action, an *Add a price you saw* form and a *How price comparison works* explainer. Navigation is unchanged (five tabs). Settings gained *Supermarkets & price comparison* (market, preferred stores, store count, minimum saving). Home and the Groceries side panel show the selected basket estimate without touching the list.
- **Engine**: `src/18-compare-engine.js` (DOM-free): store registry, normalization per kg/L/unit, models for products, store products, observations and offers, price-truth labelling, deterministic matching with a review threshold, basket options (Easiest / Recommended / Cheapest), product search, provider seams and manual price entry.
- **Data**: four new document collections (`products`, `storeproducts`, `priceobs`, `offers`) loaded per profile; the week document may carry `basketChoice`; settings gained `countryCode`, `preferredStores`, `compareMode`, `minStoreSaving`. Existing documents are untouched and `PLENTY_DATA_VERSION` stays 2. Netlify: the new collections round-trip via `app_documents`; account price-history deletion also clears `priceobs` and `offers`.
- **Tests**: `tests/compare.test.js` (9 tests) plus an extended Netlify deletion case; 56 tests in total.
- **Not live**: retailer prices. `RemoteProvider` and all `RETAILER_ADAPTERS` are planned; prices come only from confirmed receipts and manual entry until authorized sources exist behind Netlify Functions.
