...
const recipeImageDir=path.join(ROOT,'pwa','recipe-images');
const recipeImages=fs.existsSync(recipeImageDir)?fs.readdirSync(recipeImageDir).filter(f=>/\.(jpe?g|png|webp)$/i.test(f)):[];
if(recipeImages.length)fs.cpSync(recipeImageDir,path.join(dist,'recipe-images'),{recursive:true});
const precache = ['./', './index.html', './manifest.webmanifest', ...icons.map(i => './' + i), ...recipeImages.map(i=>'./recipe-images/'+i)];
